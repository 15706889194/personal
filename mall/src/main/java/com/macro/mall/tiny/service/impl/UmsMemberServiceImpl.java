...
public class UmsMemberServiceImpl implements UmsMemberService {
    ...
    public CommonResult generateAuthCode(String telephone) {
       StringBuilder stringBuilder = new StringBuilder();
        ...
        //验证码绑定手机号并存储到redis
        System.out.println("验证码是："+stringBuilder.toString());
    ...
    public CommonResult verifyAuthCode(String telephone, String authCode) {
        ...
        String string =REDIS_KEY_PREFIX_AUTH_CODE + telephone;
        String AuthCode=redisService.get(string);
        if(authCode.equals(AuthCode)){
            return CommonResult.success("验证成功");
        }else {
            return CommonResult.failed("验证失败，请重新尝试");
        }
    }
}
